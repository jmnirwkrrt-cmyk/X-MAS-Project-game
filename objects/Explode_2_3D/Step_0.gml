...
if (instance_exists(all_movables_objects_3D))
{
	for (var i = 0; i < instance_number(all_movables_objects_3D); i++)
	{
		...
		show_debug_message("Бабах!");
		if (!variable_instance_exists(inst, "hp") || !variable_instance_exists(inst, "direct")) continue;
		show_debug_message("Бабах - почти получил");
		if (point_distance_3d(x, y, z, inst.x, inst.y, inst.z) > global.range_to_hit * 2 || inst.fall > 0) continue;
		inst.flash = 0;
		inst.jump = 0;
		...
		if (inst.y < y) { inst.vel_y = 5; }
		inst.hp -= damage;
		show_debug_message("Бабах - получил");
	}
}
