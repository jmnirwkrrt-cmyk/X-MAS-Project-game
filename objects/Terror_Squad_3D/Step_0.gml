...
if (start=0) {
   if (index=="metalson") {
	  ...
	  can_jump = true; // может ли прыгать
	  can_flash = true;
	  can_shoot_plasma = false;
	  can_shoot_plasma_bombs = false;
	  suicide_man = false;
	  can_fight = true;
   ...
   if (index=="ufo") {
      ...
      jump_start_index=10; // номер начала роста z
      jump_end_index=21; // номер начала падения z
	  can_jump = false; // может ли прыгать
	  can_flash = false;
	  can_shoot_plasma = false;
...
if (instance_exists(X_MAS_team)) {
   ...
   if (instance_exists(X_MAS_team)) {
      enemy=instance_nearest(x, y, X_MAS_team) if (flash=0 and fall=0 and fight=0) {
         if (distance_to_object(enemy) > 20) {
            ...
            if (flash=0 and jump < 1 and fall=0) {
               hspeed=0;
               vspeed=0;
               status="idle";
            }
   ...
   if (jump=0 and land_dist < 100 and fight=0 and fall=0 and flash=0) {
      ...
      if (zmax < land_obj.z and (land_dist < land_obj.image_xscale + 20 or land_dist < image_yscale + 20) and instance_exists(enemy) and collision_line(x, y, enemy.x, enemy.y, all_land_objects_parent, true, true)) {
         if (x > land_obj.x) {
            if (x - land_obj.x > y - land_obj.y) {
               left=1;
               right=0;
               direct="right";
            }

            if (y - land_obj.y > x - land_obj.x) {
               up=1;
               down=0;
               direct="up";
            }
   ...
   if (left=0 and right=0 and fall=0) {
      hspeed=0;
   }

   if (hspeed=0 and vspeed=0 and status="run") {
      status="idle";
   }
...
if (instance_exists(X_MAS_team)) {
   ...
   if (fall=2) {
      ...
      if (vspeed > -0.15 and vspeed < 0.15) {
         vspeed=0;
      }

      if (hspeed > -0.15 and hspeed < 0.15) {
         hspeed=0;
      }
   }
   ...
   if (hit=2) {
      ...
      if (image_index=image_number - 1 or image_index=0) {
         hit=0;
         image_index=0;
      }
   }
...
if (instance_exists(X_MAS_team)) {
   ...
   if (instance_exists(X_MAS_team)) {
      if (status="idle" and hit=0 and flash=0 and jump < 1 and fall=0 and fight=0 and hspeed=0 and vspeed=0) {
         if (distance_to_object(enemy) < 31) {
            ...
            if (enemy.fight=0) {
               if (fight=0) {
                  if (rand < 25) {
                     fight=1 image_index=0;
                     status="fight";
                  }

                  if (rand > 25 and rand < 50) {
                     fight=8 image_index=0;
                     status="kick";
      ...
      if (flash=3) {
         ...
         if (image_index=11) {
            shell=instance_create(x, y, plasma);
            shell.damage=40;
         }
